    ...
    while time.time() - time_zero < time_of_waiting:
        ...
        try:#checks for transaction ID
            ...
            for header in h2_header:
                ...
                for div in driver.find_elements_by_tag_name("div"):
                    if "Transaction Id" in div.text and len(div.text) < 40:
                        print(" -RGPTOC: payment is complete collecting transaction id, using LEGACY METHOD")
                        return div.text.split("\n")[1]
        except:
            pass
        try:#checks for transaction ID using new method
            print(" -RGPTOC<BETA_DEBUG>: ", driver.find_element_by_xpath("/html/body/div/main/div").get_attribute("class"))
            transaction_id = driver.find_element_by_css_selector(".purchase-success").get_attribute("trxno")
            print(" -RGPTOC<BETA_DEBUG>: " + transaction_id)
            if (len(transaction_id) > 7):
                print(" -RGPTOC: payment is complete collecting transaction id, using NEW METHOD")
