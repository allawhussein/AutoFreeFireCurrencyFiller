...
def get_alcaptain_main_page(driver, window_handle, force_refresh = 0):#this funciton will take as a parameter the driver, and the handle, it does what it says
    ...
    while time.time() - time_zero < time_of_waiting:
        ...
        if len(driver.find_elements_by_class_name("session-expired")):
            print(" -GAMP: login is required, reloading")
            driver.refresh()
...
def select_order(driver, window_handle, order_phrase_list = target_order_list):
    #return (order_id, order_pubg_id, order_uc, offer_uc)
    time_of_waiting = 60
    ...
    print(" -SO: searching for Free Fire Orders")
    if len(list_of_orders) > 0:
        counter = 0
        for order in list_of_orders:
            for order_phrase in order_phrase_list:
                if order_phrase in order.find_elements_by_tag_name("td")[1].text:
                    row_id = order.find_element_by_tag_name("td").text#id of the order in alcaptain
                    #row_pubg_id = int(order.find_elements_by_tag_name("td")[4].text.split(">")[1])#pubg id
                    #removed due to website update
                    if row_id not in archive():
    ...
    if len(list_of_valid_orders) > 0:#if no requests are present, just keep refreshing
        for order in list_of_valid_orders:
            for order_phrase in order_phrase_list:
                if order_phrase in order.find_elements_by_tag_name("td")[1].text:
                    ...
                    order_uc = order.find_elements_by_tag_name("td")[1].text
                    #order_pubg_id = int(order.find_elements_by_tag_name("td")[4].text.split(">")[1])#removed due to a website update
                    order_chosen = 1
                    break
            if order_chosen == 1:
                break
        ...
        elif "Free" in order_uc:
            print(" -SO: no offer with order")
            offer_uc = 0
            order_uc = int(order_uc.split(" ")[0])
        else:
            print(" -SO: Unrecognized Format: ", order_uc)
            print(" -SO: Order ID: ", order_id)
            return None
        
        print(" -SO: selecting order of AlcaptainID: ", order_id)
        ...
        for input_field in driver.find_elements_by_tag_name("input"):
            if input_field.get_attribute("placeholder") == "Type ID Player":
                order_pubg_id = input_field.get_attribute("value")
                break
        else:
            print(" -SO: Player ID wasn't obtained")
            return None
        ...
        print("     |-Free Fire ID " + str(order_pubg_id))
        ...
        for order in list_of_orders:
            for order_phrase in order_phrase_list:
                if order_phrase in order.find_elements_by_tag_name("td")[1].text:
                    ...
                    if row_id in archive():
                        list_of_archived_orders.append((row_id, str(row_pubg_id)))
                        break
    if len(list_of_archived_orders):
        print(" -GLOAAO: " + str(len(list_of_archived_orders)) + " archived Free Fire orders are found, sending new stall order to through telegram")
    else:
